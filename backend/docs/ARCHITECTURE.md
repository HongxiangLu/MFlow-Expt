...
*   **`graph_service.py`**: 负责处理知识图谱渲染接口。**独立执行 Query Rewrite**，与 Chat 接口彻底解耦。
    ...
    *   **Query Rewrite System Prompt**：
        ...
        ```
    *   **Query Rewrite 稳定性优化（防“回答化”）**：
        1. **输入编排改造**：不再将历史消息逐条作为 chat 多轮消息传入（该形态容易触发模型“继续对话”并直接回答），而是改为 `system + 单条 user任务指令`。其中 user 指令内部结构化包含「规则 + 历史摘要 + 当前提问」。
        2. **历史窗口收敛**：仅截取最近窗口（当前实现为最近 12 条消息）参与重写，降低远端噪声对指代消解的干扰。
        3. **确定性采样**：重写调用固定 `temperature=0.0`，减少解释性扩写和风格漂移。
        4. **结果后处理**：对输出执行 `<think>` 清理与单行化（压缩空白为单空格），保证下游检索 query 形态稳定。
        5. **防御性兜底**：若返回结果呈现明显“回答腔”（如“答案/根据/通常/可以/首先/这个问题”开头）或长度异常（远超重写句预期），回退到原始 query，避免回答文本污染图谱检索。
        6. **目标边界**：重写仅做“最小必要改写”（代词消解/省略补全），不引入新事实，不改变用户意图。
